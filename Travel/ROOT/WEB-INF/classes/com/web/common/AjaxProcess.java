...
public class AjaxProcess extends Generic 
{
	...
	public boolean setUserInfo(UploadJava upld) throws SQLException,Exception
	{
		ret =false ;
		...
		if(  upld != null && upld.isUpdated() && ret )
		{
	...
	public String updateUserImage(String userId) throws SQLException
	{
		...
		String ImagePath = "Images/profileImages/"+userId ;
		
		int fileSize			 = isNull(request.getParameter("fileSize")).equals("") ? 0 : Integer.parseInt(isNull(request.getParameter("fileSize"))) ; 
		...
		ui.setImage_name(fileName);
		ui.setImage_path(ImagePath);
		ui.setUploaded_by(userId);
		ui.setUser_id(userId);
		ui.setSize( fileSize) ;
		ui.setType(fileType) ;
		ui.setObjectUrl(ObjectUrl);
		...
		if( ! uim.isImageExists(ui))
		{
			if( ret = uim.insertUser(ui) )
				session.setAttribute("ImagePath", request.getParameter("objectUrl"));
		}
		else
		{
