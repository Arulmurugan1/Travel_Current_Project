...
public class AjaxServlet extends CustomServlet 
{
	...
	{ 
		...
		try
		{
			...
			{
				boolean  isFileChanged = CommonFactory.isNull (request.getParameter("isFileChanged")).equals("true") ;
				
				logContent("isFileChanged >" +isFileChanged, LoggerFactory.DEBUG);
				
				String message = "";
				
				if(isFileChanged)
				{
					message += " Image Updation " + ret.updateUserImage(null) +"\n";
					logContent(message, LoggerFactory.DEBUG);
				}
				
				message += " Info Updation " + ( ret.setUserInfo(null) ? " Sucess .. " : " Failed ...");
				logContent(message, LoggerFactory.DEBUG);
				out.print(message);
			...
			else
			{
				...
				else 
				{
					...
					if ( mode.trim().equals("getVehicleStatus"))
					{
					...
					if ( mode.trim().equals("getVehicles"))
					{
