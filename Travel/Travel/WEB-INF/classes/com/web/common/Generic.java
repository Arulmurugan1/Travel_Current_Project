...
public class Generic extends StringChecker
{
    ...
    public void closeAll() throws Exception
    {
        ...
        logContent(sb.toString() ,LoggerFactory.DEBUG , null, this);
    }
    ...
    public static void closeAll(Connection con,PreparedStatement ps , ResultSet rs,Object object) throws Exception
    {
        ...
        logContent(sb.toString() ,LoggerFactory.DEBUG , null, object);
    }
    ...
    public static void InsertAccessLog(HttpServletRequest req) throws SQLException, IllegalArgumentException, IllegalAccessException 
    {
        ...
        logContent("Access Log Entered "+( ps.executeUpdate() > 0 ) ,LoggerFactory.DEBUG , null, Generic.class );
    	...
    	if(isLoggable)
    	{
    		if (session.getAttribute("ConsoleLog") == null )
    		{
    			try 
    			{
    				...
    				if ( f.exists() ) 
    				{   
    					logContent("Get File Path for console log=> " + f.getAbsolutePath() , LoggerFactory.DEBUG, null, Generic.class);
    				...
    				else 
    				{
    					...
    					if ( f.createNewFile() ) 
    					{
    						logContent("File created " + f.exists() + " File Name :" + f.getName() , LoggerFactory.DEBUG, null, Generic.class);
    						logContent("Get File Path => " + f.getAbsolutePath() , LoggerFactory.DEBUG, null, Generic.class);
    					...
    					{
    						logContent("File Creation Issue" ,  LoggerFactory.DEBUG, null, Generic.class);
    					}
    				}

    			} catch (Exception e) 
    			{
    				logContent("Exception in File Path Creation =>"+e.getMessage() ,LoggerFactory.ERROR , null, Generic.class );
    				return;
    			}
    		}
    		...
    		logContent("Get PrintStream [" + printStream +"Property ["+printProperty+"]" , LoggerFactory.DEBUG, null, Generic.class);
    ...
    public static void setFilePath(HttpSession session) throws Exception 
    {
        ...
        if ( session.getAttribute("consoleFilePath") == null )
        {
            ...
            logContent("Log File Path =>"+loggerFilePath ,LoggerFactory.DEBUG , null, Generic.class );

            session.setAttribute("consoleFileName", consoleFileName);
            session.setAttribute("loggerFilePath", loggerFilePath);
            
            logContent(" File Path Added to session " ,LoggerFactory.DEBUG , null, Generic.class );
        }
